...
export const uploadImageController = async (req: Request, res: Response, next: NextFunction) => {
  ...
  return res.json({
    message: USERS_MESSAGES.UPLOAD_SINGLE_IMAGE_SUCCESS,
    data: result
  })
}
...
export const severImageController = (req: Request, res: Response, next: NextFunction) => {
  const { name } = req.params
  return res.sendFile(path.resolve(UPLOAD_IMAGE_DIR, name), (err) => {
    if (err) {
      res.status(err as any).send({
